...
class AccountAgedSeller(models.Model):
    _name = 'account.aged.receivable.seller'

    _inherit = 'account.aged.receivable'
    _description = "Aged Receivable By Seller"

    seller_id = fields.Many2one('res.partner')
    # Dummy field to resolve addons conflict
    invoice_user_name = fields.Char(string="Seller", group_operator='max')

    period1 = fields.Monetary(string='1 - 7')
    period2 = fields.Monetary(string='8 - 14')
    period3 = fields.Monetary(string='15 - 30')
    period4 = fields.Monetary(string='31 - 60')
    ...
    def build_sql(self):
        sql = """
            SELECT
                ...
                seller.name AS invoice_user_name,
            ...
            JOIN res_partner seller ON seller.id = res_users.partner_id
            ...
            GROUP BY account_move_line.id, partner.id, seller.name, seller.id,trust_property.id, journal.id, move.id, account.id,
    ...
    def _get_query_period_table(self, options):
        ...
        period_values = [
            ...
            (minus_days(date, 8), minus_days(date, 14)),
    ...
    def _get_hierarchy_details(self, options):
        return [
            self._hierarchy_level('seller_id', foldable=True, namespan=5),
            self._hierarchy_level('id'),
        ]

    def _format_seller_id_line(self, res, value_dict, options):
        res['name'] = value_dict['invoice_user_name'][:128] if value_dict['invoice_user_name'] else _('Unknown Seller')

    @api.model
    def _get_report_name(self):
        return _("Aged Receivable By Seller")
