...
def validate_token(func):
    ...
    def wrap(self, *args, **kwargs):
        ...
        if access_token_data.find_one_or_create_token(
            ...
            return invalid_response("access_token",
...
class APIController(http.Controller):
    ...
    def get(self, model=None, id=None, **payload):
        try:
            ioc_name = model
            model = request.env[self._model].search([("model", "=", model)],
                                                    limit=1)
            if model:
                domain, fields, offset, limit, order = \
                    extract_arguments(payload)
    ...
    @validate_token
    @http.route(_routes, type="http", auth="none",
    ...
    def post(self, model=None, id=None, **payload):
        ...
        return \
            invalid_response("Error",
                             "Error occurs when try to request information.")
    ...
    def put(self, model=None, id=None, **payload):
        ...
        if not _model:
            return invalid_response(
                "invalid object model",
                "The model %s is not available in the registry." % model, 404)
        try:
    ...
    def delete(self, model=None, id=None, **payload):
        ...
        try:
            ...
            return \
                invalid_response("invalid object id",
                                 "invalid literal %s for id with base " % id)
        try:
            ...
            return \
                valid_response(
                    "record %s has been successfully deleted" % record.id)

    @validate_token
    @http.route(_routes, type="http",
                auth="none", methods=["PATCH"], csrf=False)
    def patch(self, model=None, id=None, action=None, **payload):
        ...
        try:
            ...
            return invalid_response("invalid object id",
                                    "invalid literal %s for id with base" % id)
        try:
            record = request.env[model].sudo().search([("id", "=", _id)])
            ...
            if record and _callable:
                ...
                getattr(record, action)(*args) \
                    if args else getattr(record, action)()
            else:
                return \
                    invalid_response(
                        "missing_record",
                        "record id %s  not found or %s no method %s" % (
                            _id, model, action),
                        404,
                    )
        except Exception as e:
            ...
            return valid_response(
                "record %s has been successfully update" % record.id)
