...
class AccessToken(http.Controller):
    ...
    @http.route("/api/auth/token/v1", methods=["GET"], type="http",
                auth="none", csrf=False)
    def token_v1(self, **post):
        ...
        Returns:
            ...
            returns https response code 404 if failed error message
        ...
        Example:
           ...
           eq = requests.post(
               '{}/api/auth/token'.format(base_url), data=data,
               headers=headers)
        ...
        _credentials_includes_in_body = all([db, username, password])
        if not _credentials_includes_in_body:
            # The request post body is empty
            # the credetials maybe passed via the headers.
            headers = request.httprequest.headers
            db = headers.get("db")
            username = headers.get("login")
            password = headers.get("password")
            ...
            if not _credentials_includes_in_headers:
                ...
                return invalid_response(
                    "missing error",
                    "Following are missing [db, username,password]",
                    403,
                )
        # Login in odoo database:
        try:
        ...
        return werkzeug.wrappers.Response(
            ...
            response=json.dumps(
                {
                    "uid": uid,
                    "user_context": request.session.get_context() if uid
                    else {},
                    "company_id": request.env.user.company_id.id if uid
                    else None,
